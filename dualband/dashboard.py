...
<html>
<head>
    <title>Echo-Alverca — Dual-Band Dashboard</title>
    ...
    <style>
        ...
        .card { background:#1a1d27; border-radius:10px; padding:18px;
                min-width:160px; flex:1; border:1px solid #2a2d3a; }
        .card h3 { color:#888; font-size:0.8em; text-transform:uppercase; margin-bottom:8px; }
        .big  { font-size:2.2em; font-weight:bold; }
        .c433 { color:#ff6b35; } .c868 { color:#00d4ff; }
        .cboth{ color:#7cfc00; } .ctot { color:#fff; }
        table { width:100%; border-collapse:collapse; margin-bottom:25px; font-size:0.88em; }
        th    { background:#1a1d27; padding:9px 13px; text-align:left;
                color:#00d4ff; border-bottom:2px solid #2a2d3a; }
        ...
        .b433 { background:#ff6b3533; color:#ff6b35; }
        .b868 { background:#00d4ff22; color:#00d4ff; }
        .bboth{ background:#7cfc0022; color:#7cfc00; }
        .bmiss{ background:#ff222233; color:#ff4444; }
        .ts   { color:#666; font-size:0.8em; }
    </style>
</head>
<body>
    <h1>📡 Echo-Alverca — Dual-Band Monitor</h1>
    <p class="sub">Auto-refreshes every 30s &nbsp;|&nbsp; LI.VFX Alverca</p>

    <h2>Overall Stats</h2>
    <div class="grid">
        <div class="card">
            <h3>Total Messages</h3>
            <div class="big ctot">{{ s.get('total', 0) }}</div>
        </div>
        <div class="card">
            <h3>433 MHz</h3>
            <div class="big c433">{{ s.get('received_433', 0) }}</div>
            <small>{{ s.get('success_rate_433', 0) }}% success</small>
        </div>
        <div class="card">
            <h3>868 MHz</h3>
            <div class="big c868">{{ s.get('received_868', 0) }}</div>
            <small>{{ s.get('success_rate_868', 0) }}% success</small>
        </div>
        <div class="card">
            <h3>Both Bands</h3>
            <div class="big cboth">{{ s.get('received_both', 0) }}</div>
            <small>{{ s.get('success_rate_both', 0) }}% success</small>
        </div>
    </div>

    <h2>Per Channel</h2>
    <table>
        <tr><th>Channel</th><th>Total</th><th>433 MHz</th><th>868 MHz</th><th>Both</th></tr>
        {% for ch in channels %}
        <tr>
    ...
    <table>
        <tr>
            <th>Time</th><th>Channel</th><th>Sender</th>
            <th>Message</th><th>First Band</th><th>Both?</th>
        </tr>
        {% for m in recent %}
        <tr>
            <td class="ts">{{ m.readable_ts }}</td>
            ...
            <td>
                {% if m.first_band == '433' %}
                    <span class="b b433">433 MHz</span>
                {% else %}
                    <span class="b b868">868 MHz</span>
                {% endif %}
            </td>
            <td>
                {% if m.received_433 and m.received_868 %}
                    <span class="b bboth">✓ Both</span>
                {% else %}
                    <span class="b bmiss">One only</span>
                {% endif %}
            </td>
        </tr>
        {% else %}
